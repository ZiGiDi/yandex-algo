...
//    Найдите кратчайшее расстояние между парой вершин в неориентированном графе. Граф может быть несвязным.
    ...
    public static void main(String[] args) throws IOException {
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(System.in));
            ...
            List<Integer> vertexInfo = readList(reader);
            int startVertexes = vertexInfo.get(0);
            int finishVertexes = vertexInfo.get(1);

            runBFS(numberOfVertexes, startVertexes);
            writeSolution(finishVertexes, writer);
        }
    }
    ...
    private static void writeSolution(int finishVertex, BufferedWriter writer) throws IOException {
        Integer distance = F.distance.get(finishVertex - 1);
